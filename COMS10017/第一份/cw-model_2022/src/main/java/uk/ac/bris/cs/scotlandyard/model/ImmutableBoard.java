...
public class ImmutableBoard implements Board, Serializable {
	...
	public ImmutableBoard(Board that) {
		this.setup = Objects.requireNonNull(that.getGameSetup());
		this.detectiveLocations = Objects.requireNonNull(that.getPlayerList().stream()
		...
		this.tickets = Objects.requireNonNull(
				that.getPlayerList().stream().collect(ImmutableMap.toImmutableMap(
						Function.identity(), x -> {
	...
	@Nonnull @Override public GameSetup getGameSetup() { return setup; }
	@Nonnull @Override public ImmutableSet<Piece> getPlayerList() { return tickets.keySet(); }
