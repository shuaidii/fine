...
public class Enemy implements GameObject, Renderable {
    ...
    public Enemy clone() throws CloneNotSupportedException {
        ...
        enemy.enemyProjectile = new ArrayList<>();
        enemy.pendingToDeleteEnemyProjectile = new ArrayList<>();
        for (Projectile projectile : enemyProjectile) {
            Projectile clone = projectile.clone();
            enemy.enemyProjectile.add(clone);
            if (!projectile.isAlive()) {
                enemy.pendingToDeleteEnemyProjectile.add(clone);
            }
        }
        return enemy;
    }
